...
class Surveie extends Model
{
    ...
    public function auth($auth_model, $auth){
        if(session()->put('authoritys') == null){
            $authority = $auth_model::where('id', $auth::user()->auth_id)->select('authority')->get();
    ...
    public function log_all($number,$name,$action){
        ...
        $logs = new Log;
        $format = '調査会社管理、管理ID：%d、会社名：%sを%sしました。';
