...
<div class="container">
    ...
    @if ($errors->any())
        <div class="alert-danger csv-message">
            ...
            @php 
                ...
                $search = ['values.', '.survey_name', '.member', '.contractor', '.address', '_contact', '。'];
                $replace = ['', '', '', '', '', '', "。<br>"];
    ...
    <form action="{{ action('ClientController@csv_import_check') }}" method="POST" enctype="multipart/form-data">
    {{ csrf_field() }}
        <input type="file" class="form-control-file" name="csv" required>
        <input type="submit" value="インポート" class="btn btn-info">
    </form>
</div>
@endsection